...
def hourglass(window, n, point, radius, color):
    ...
    for i in range(n):
        ...
        x = point.x - radius * (i + 1)
        y = y - math.sqrt(3) * radius
        x2 = point.x - radius * (i + 1)
...
def many_hourglasses(window, square, m, colors):
    ...
    new_colors = colors
    new_colors = new_colors * (m // len(new_colors) + 1)
    # new_colors = colors
    # while len(new_colors) < m:
    #     new_colors = new_colors * 2

    for i in range(m):
        rect = rg.Rectangle(rg.Point(x - (i + 1) * radius, y - i * h - radius),
                            rg.Point(x + (i + 1) * radius, y + i * h + radius))
        rect.attach_to(window)
        window.render()
        hourglass(window, i + 1, rect.get_center(), radius, new_colors[i])
